...
2. 分层固定为 `Core DTO → Presentation Adapter → AgentNaviView → MCP Apps`；
...
6. 协议输出递归限制为严格 JSON-safe 值，所有路径必须是 POSIX 项目相对路径；所有 wire 字符串都会检查绝对路径 canary，绝对路径和敏感内部字段在序列化边界 fail closed；
7. `Error` 供 MCP 错误映射使用，不加入成功 Envelope；
8. 协议 DTO 只使用 Python 标准库，不导入 MCP SDK。MCP Server 通过 optional extra 和 lazy import 在后续实现；
9. 新增可选 extension 字段必须向后兼容且不能覆盖保留字段；删除、改名、改类型或改变语义时升级 `schemaVersion`；
...
- 所有 view 共享来源、证据、warning、路径和隐私规则；
...
- 新字段必须维护兼容性和稳定排序；
- fail-closed 输出可能把内部脏数据变成明确错误，需要 Server 映射为可操作提示；
