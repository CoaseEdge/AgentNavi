...
class MCPSetupContractTestCase(unittest.TestCase):
    ...
    def test_codex_setup_is_a_toml_fragment(self) -> None:
        stdout = io.StringIO()
        with contextlib.redirect_stdout(stdout):
            result = main(["mcp", "setup", "--target", "codex"])
        self.assertEqual(result, 0)
        self.assertIn("[mcp_servers.agentnavi]", stdout.getvalue())

    def test_vscode_setup_declares_stdio_type(self) -> None:
        config = build_setup_config(target="vscode")
        self.assertEqual(config["servers"]["agentnavi"]["type"], "stdio")
