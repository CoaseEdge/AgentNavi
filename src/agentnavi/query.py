...
def task_detail_data(
    ...
    *,
    ...
    """读取一个任务的安全 L3 摘要，不读取 prompt、events 或 edge.data_json。

    关系按 edge 的追加顺序稳定读取；无效目标在展示上限前过滤。分页和总扫描量
    均为常量，避免损坏行把后续有效事实静默挤出，也避免退化为全图扫描。
    """

    if not isinstance(task_id, str) or not task_id.strip() or relation_limit < 1:
        return None
    display_limit = min(int(relation_limit), 20)
    page_size, max_pages = 32, 8
    with database.connect() as connection:
        connection.execute("BEGIN")
        task = connection.execute(
            """SELECT /* task-detail-task */ task.id, task.title, task.status,
                      task.summary, task.created_at, task.updated_at, task.closed_at,
                      node.id AS node_id, node.source AS node_source,
        ...
        if task is None or task["node_source"] != "task-events" or any(
            contains_private_path(str(task[field] or ""))
        ...
        for page in range(max_pages):
            raw = connection.execute(
                """SELECT /* task-detail-edge-page */ id, rowid AS recorded_order
                   FROM edges INDEXED BY idx_edges_source
                   WHERE project_id=? AND layer=3 AND source_id=?
                     AND (? IS NULL OR rowid < ?)
                   ORDER BY rowid DESC LIMIT ?""",
                (project["id"], task["node_id"], cursor, cursor, page_size),
            ).fetchall()
            if not raw:
                exhausted = True
                break
            ...
            hydrated = connection.execute(
                f"""SELECT /* task-detail-edge-hydrate */ edge.id, edge.source_id,
                           edge.target_id, edge.relation, edge.source,
                           edge.confidence, target.layer AS target_layer,
                           target.kind AS target_kind, target.key AS target_key,
                           target.label AS target_label, target.source AS target_source,
                           target.confidence AS target_confidence
                    FROM edges edge JOIN nodes target ON target.id=edge.target_id
                    WHERE edge.id IN ({placeholders})""",
                ids,
            ).fetchall()
            by_id = {str(row["id"]): row for row in hydrated}
            for raw_row in raw:
                row = by_id.get(str(raw_row["id"]))
                if row is None or row["source"] != "task-events" or row["relation"] not in {
                    "read", "modified", "tested", "searched", "affects"
                }:
                    filtered = True
                    continue
                ...
                if not (is_file or is_concept) or contains_private_path(str(row["target_source"])) or (
                    path is not None and not is_canonical_relative_path(path)
                ) or contains_private_path(str(row["target_label"])):
                    filtered = True
                    continue
                evidence = _context_evidence(
                    kind="task-relation",
                    summary=f"任务图记录 {row['relation']} 关系。",
                    layer="L3",
                    source="task-events",
                    confidence=float(row["confidence"]),
                    path=path,
                ...
                relations.append({
                    "entity": {
                        ...
                        **({"path": path} if path is not None else {}),
                        "layer": f"L{int(row['target_layer'])}",
                        "source": str(row["target_source"]),
                        "confidence": float(row["target_confidence"]),
                ...
                if len(relations) > display_limit:
                    break
            if len(relations) > display_limit:
                break
    ...
    summary = " ".join(str(task["summary"] or "").split())[:480]
    title = " ".join(str(task["title"]).split())[:240]
    return {
        "id": str(task["id"]),
        "nodeId": str(task["node_id"]),
        "title": title,
        "status": str(task["status"])[:40],
        "summary": summary,
        "createdAt": str(task["created_at"]),
        "updatedAt": str(task["updated_at"]),
        "closedAt": str(task["closed_at"]) if task["closed_at"] else None,
        "sortTime": str(task["closed_at"] or task["updated_at"] or task["created_at"]),
        "source": str(task["node_source"]),
        "confidence": float(task["node_confidence"]),
        "relations": relations[:display_limit],
        "relationsTruncated": len(relations) > display_limit or not exhausted,
